...
st.markdown("4. กดปุ่มเพื่อสร้างตาราง ระบบจะจัดกลุ่มหัวตาราง ตีกรอบแบ่งเจ้า และไฮไลท์สีเขียวที่ช่อง **Unit Price** ของเจ้าที่ถูกที่สุดให้ทันที")
...
if uploaded_files:
    ...
    for idx, file in enumerate(uploaded_files):
        ...
        with cols[idx]:
            v_name = st.text_input(f"🏢 เจ้าที่ {idx+1} (จากไฟล์ {file.name[:15]}...):", value=default_name, key=f"vendor_{idx}")
    ...
    if not api_key:
        ...
        try:
            ...
            with st.spinner("⏳ AI กำลังอ่านไฟล์และจัดทำตารางเปรียบเทียบราคา โปรดรอสักครู่..."):
                ...
                vendor_names_str = ", ".join([f"'{name}'" for name in vendor_names])
                
                prompt = f"""คุณคือผู้ช่วยฝ่ายจัดซื้อมืออาชีพที่มีความแม่นยำสูงมาก ฉันส่งไฟล์ใบเสนอราคามาให้คุณจำนวน {len(uploaded_files)} ไฟล์ ซึ่งตัวแทนบริษัทผู้ขายตามลำดับดังนี้: {vendor_names_str}
...
- ตรวจสอบตัวเลขให้ถูกต้องแม่นยำ 100% ตามหน้าเอกสาร"""
                ...
                # 7. สร้างตารางแบบ Multi-Index (หัวตาราง 2 ชั้น แยก Vendor ชัดเจน)
                table_data = {}
                ...
                sum_row[('', 'ลำดับ')] = ""
                sum_row[('', 'รายการ')] = "ราคารวม (Total)"
                sum_row[('', 'ขนาด')] = ""
                sum_row[('', 'หน่วย')] = ""
                
                vat_row[('', 'ลำดับ')] = ""
                vat_row[('', 'รายการ')] = "Vat. 7%"
                vat_row[('', 'ขนาด')] = ""
                vat_row[('', 'หน่วย')] = ""
                
                net_row[('', 'ลำดับ')] = ""
                net_row[('', 'รายการ')] = "ราคารวมภาษี (Net Total)"
                net_row[('', 'ขนาด')] = ""
                net_row[('', 'หน่วย')] = ""
                
                all_sections = [f"🏢 {name}" for name in vendor_names] + ["⭐ MIN"]
                
                for sec in all_sections:
                    u_col = (sec, "Unit Price")
                    amt_col = (sec, "Amount")
                ...
                # 9. ฟังก์ชันตกแต่งสี ตีกรอบ และไฮไลท์ช่องถูกที่สุด (ไม่แปลงเป็น string ก่อน เพื่อไม่ให้บั๊ก)
                def style_excel_layout(df_in):
                    styles = pd.DataFrame('', index=df_in.index, columns=df_in.columns)
                    
                    # 1. ตีกรอบแนวตั้งแยกแต่ละ Vendor และสลับสีพื้นหลังอ่อนๆ เพื่อไม่ให้ตาลาย
                    for col in df_in.columns:
                        top_header = col[0]
                        sub_header = col[1]
                        base_style = ''
                        
                        # เส้นทึบแบ่งหน้าเมื่อขึ้น Vendor ใหม่
                        if sub_header == 'Unit Price':
                            base_style += 'border-left: 3px solid #4A5568 !important; '
                            
                        # สลับสีพื้นหลังของแต่ละ Vendor (คี่=สีฟ้าอ่อนมาก, คู่=สีขาว)
                        if '🏢' in top_header:
                            clean_name = top_header.replace('🏢 ', '').strip()
                            if clean_name in vendor_names:
                                v_num = vendor_names.index(clean_name)
                                if v_num % 2 == 1:
                                    base_style += 'background-color: #F8FAFC; ' # สีเทา/ฟ้าอ่อนมาก
                                else:
                                    base_style += 'background-color: #FFFFFF; '
                        elif 'MIN' in top_header:
                            base_style += 'background-color: #F1F5F9; color: #475569; ' # พื้นหลังเทาสำหรับช่อง Min
                            
                        styles[col] = base_style
                    
                    # 2. ตกแต่งแถวสรุป 3 บรรทัดสุดท้าย
                    for r_idx in range(len(df_in) - 3, len(df_in)):
                        for col in df_in.columns:
                            styles.loc[r_idx, col] += 'font-weight: bold; border-top: 1px solid #A0AEC0; background-color: #E2E8F0 !important;'
                            if 'MIN' in col[0]:
                                styles.loc[r_idx, col] += 'background-color: #CBD5E1 !important; color: #0F172A;'
                    
                    # 3. ไฮไลท์สีเขียวเข้ม "เฉพาะช่อง Unit Price และ Amount ของ Vendor เจ้าที่ราคาถูกที่สุด"
                    for r_idx in range(len(df_in) - 3):
                        ...
                        if isinstance(min_val, (int, float)) and min_val > 0:
                            for name in vendor_names:
                                ...
                                # เช็คตัวเลขด้วยความละเอียด เพื่อไม่ให้พลาดการไฮไลท์
                                if isinstance(val, (int, float)) and abs(val - min_val) < 1e-4:
                                    styles.loc[r_idx, u_col] += 'background-color: #C6E0B4 !important; color: #006100 !important; font-weight: bold; border-left: 3px solid #006100 !important;'
                                    styles.loc[r_idx, amt_col] += 'background-color: #C6E0B4 !important; color: #006100 !important; font-weight: bold; border-right: 2px solid #006100 !important;'
                                    
                    return styles

                st.success("✅ สร้างตารางเปรียบเทียบรูปแบบ Excel สำเร็จ!")
                st.markdown("### 📋 ตารางเปรียบเทียบราคา (ตีกรอบแยกเจ้าและไฮไลท์ราคาถูกสุด)")
                
                # จัดการ Format ทศนิยมโดยใช้ .format() ของ Styler เพื่อไม่ให้กระทบกับการเช็คสี
                format_dict = {}
                for col in final_display_df.columns:
                    if "Unit Price" in col[1] or "Amount" in col[1]:
                        format_dict[col] = lambda x: f"{x:,.2f}" if isinstance(x, (int, float)) else str(x if x is not None else "")
                        
                styler = final_display_df.style.apply(style_excel_layout, axis=None).format(format_dict)
