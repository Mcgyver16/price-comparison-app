...
st.markdown("4. กดปุ่มเพื่อสร้างตาราง โดยระบบจะไฮไลท์สีเขียวที่ช่อง **Unit Price** ของเจ้าที่ถูกที่สุดให้ทันที")
...
# 3. ส่วนตั้งชื่อ Vendor (จะแสดงขึ้นมาทันทีเมื่อมีการอัปโหลดไฟล์)
vendor_names = []
if uploaded_files:
    ...
    # สร้างคอลัมน์ตามจำนวนไฟล์ที่อัปโหลด
    ...
    for idx, file in enumerate(uploaded_files):
        ...
        with cols[idx]:
            ...
            # ป้องกันกรณีผู้ใช้ตั้งชื่อซ้ำกัน
    ...
    if not api_key:
        ...
        try:
            ...
            with st.spinner("⏳ AI กำลังอ่านไฟล์และจัดทำตารางเปรียบเทียบราคา โปรดรอสักครู่..."):
                ...
                # โครงสร้าง JSON โดยใช้ชื่อ Vendor ที่ผู้ใช้พิมพ์กรอกเข้ามา
...
- ตรวจสอบตัวเลขให้ถูกต้องแม่นยำ 100% ตามหน้าเอกสาร"""
                ...
                # 5. ประมวลผลราคาหลังหักส่วนลด และคำนวณ Amount (Qty * UnitPrice)
                ...
                # 6. คำนวณหาช่อง MIN (Unit Price และ Amount)
                min_unit_list = []
                min_amount_list = []
                ...
                # 9. ฟังก์ชันตกแต่งสี (Highlight เฉพาะช่องของ Vendor ที่ถูกที่สุด ไม่ไฮไลท์ช่อง MIN)
                def style_excel_layout(df_in):
                    styles = pd.DataFrame('', index=df_in.index, columns=df_in.columns)
                    
                    # 1. ไฮไลท์แถวสรุป 3 แถวสุดท้ายเป็นสีเทาอ่อนและตัวหนา
                    for r_idx in range(len(df_in) - 3, len(df_in)):
                        styles.iloc[r_idx, :] = 'background-color: #f2f2f2; font-weight: bold;'
                        # ทำให้ช่อง MIN ในแถวสรุปเด่นขึ้นมานิดนึงด้วยสีเทาเข้มขึ้น
                        for col in df_in.columns:
                            if "MIN" in col:
                                styles.iloc[r_idx][col] = 'background-color: #e6e6e6; font-weight: bold; color: #000000;'
                    
                    # 2. ตรวจสอบและไฮไลท์สีเขียวเข้ม "เฉพาะในช่องของ Vendor เจ้าที่ราคาถูกที่สุด"
                    for r_idx in range(len(df_in) - 3):
                        ...
                        if isinstance(min_val, (int, float)) and min_val > 0:
                            for name in vendor_names:
                                ...
                                # ถ้า Unit Price ของเจ้านี้ เท่ากับราคา Min ให้ไฮไลท์สีเขียวทันที!
