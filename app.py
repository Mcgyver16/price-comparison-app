...
# 2. ส่วนตั้งค่าระบบด้านซ้ายมือ (เพิ่มระบบเลือกรุ่น Pro สำหรับอ่าน PDF แม่นยำ)
...
model_level = st.sidebar.radio(
    "กรณีอ่าน PDF แล้วเพี้ยน ให้เปลี่ยนเป็นรุ่น Pro ครับ:",
    ["⚡ รุ่นเร็ว (Flash - สำหรับรูปภาพ/บิลทั่วไป)", "🧠 รุ่นฉลาดล้ำลึก (Pro - แม่นยำสูงมาก สำหรับ PDF ซับซ้อน)"],
    index=1 # ตั้งค่าเริ่มต้นเป็น Pro เพื่อความแม่นยำในการอ่าน PDF
)
...
if st.button("🚀 3. เริ่มวิเคราะห์และสร้างตารางเปรียบเทียบ") and uploaded_files:
    if not api_key:
        ...
        try:
            ...
            # เลือกรุ่น AI ตามที่ผู้ใช้เลือกในแถบซ้ายมือ
            if "Pro" in model_level or "pro" in model_level:
            ...
            # หากหารุ่นที่เลือกไม่เจอ ให้ใช้รุ่นแรกที่ระบบรองรับ
            ...
            with st.spinner("⏳ AI กำลังอ่านแยกเอกสารทีละบริษัทอย่างละเอียด โปรดรอสักครู่... (รุ่น Pro อาจใช้เวลา 20-40 วินาที)"):
                
                # สร้างโครงสร้าง JSON ให้ AI ตอบกลับมา
                ...
                # --> แก้ปัญหา PDF เพี้ยน: ใส่ป้ายคั่นเอกสาร (Interleaved Text & Media) <--
                contents = []
...
หน้าที่ของคุณคือ:
1. อ่านข้อมูลรายการสินค้า จำนวน หน่วยนับ และราคาต่อหน่วย (Unit Price) จากทุกเอกสารให้ครบถ้วนทุกหน้า (กรณี PDF มีหลายหน้า)
...
- ตรวจสอบตัวเลขให้ถูกต้องแม่นยำ 100% ตามหน้าเอกสาร"""
                ...
                # วนลูปแนบไฟล์ พร้อมใส่ป้ายบอก AI ชัดเจนว่าไฟล์นี้เป็นของ Vendor ไหน
                for idx, file in enumerate(uploaded_files):
                    bytes_data = file.read()
                    
                    # บังคับเช็คชนิดไฟล์ให้ถูกต้อง 100% (แก้ปัญหา Streamlit ส่ง MIME Type ผิดพลาด)
                ...
                # ส่งข้อมูลทั้งหมดไปให้ Gemini ประมวลผล
                ...
                # 7. สร้างตารางแบบ Flat Columns เพื่อความเสถียร 100% ไม่เพี้ยนไม่ติดดำ
                table_data = {}
