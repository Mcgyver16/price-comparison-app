...
st.markdown("3. **พิมพ์กำหนดชื่อ Vendor** ของแต่ละไฟล์ตามต้องการ (ตั้งชื่อยาวหรือมีวงเล็บได้ตามสบายครับ)")
st.markdown("4. กดปุ่มเพื่อสร้างตาราง ระบบจะดึงข้อมูลด้วยรหัสย่อหลังบ้าน ทำให้ราคาขึ้นครบ 100% ไม่มีหายแน่นอน")
...
# 3. ส่วนตั้งชื่อ Vendor (รองรับชื่อยาว มีวงเล็บ หรือภาษาไทยได้สบายไม่เออเร่อ)
vendor_names = []
vendor_ids = [] # รหัสย่อหลังบ้าน v1, v2, v3... เพื่อป้องกัน AI เขียนคีย์ผิด
if uploaded_files:
    ...
    if not api_key:
        ...
        try:
            ...
            with st.spinner("⏳ AI กำลังอ่านแยกเอกสารทีละบริษัทอย่างละเอียด โปรดรอสักครู่..."):
                ...
                # --> ใช้รหัสย่อ v1, v2, v3 เป็นคีย์ JSON เพื่อให้ AI ไม่มีความสับสนหรือตัดคำ <--
                vendor_structure = ", ".join([
                    f'"{v_id}_UnitPrice": "ราคาต่อหน่วยก่อน VAT ของรหัส {v_id} (ตัวเลขทศนิยม ถ้าไม่มีสินค้านี้ใส่ null, ถ้าเขียนว่า include หรือ ฟรี ให้ใส่ข้อความว่า include)", '
                    f'"{v_id}_DiscountStr": "ส่วนลดของรหัส {v_id} เช่น \'43%\', \'100\', \'0\' (ถ้าไม่มีให้ใส่ \'0\')"'
                    for v_id in vendor_ids
                ])
                
                contents = []
                prompt_intro = f"""คุณคือผู้ช่วยฝ่ายจัดซื้อมืออาชีพที่มีความแม่นยำสูงมาก ฉันส่งไฟล์ใบเสนอราคามาให้คุณจำนวน {len(uploaded_files)} เอกสาร โดยได้กำหนดรหัสอ้างอิงเอกสารเป็น v1, v2, v3... ตามลำดับ
...
2. ทำการจับคู่สินค้าที่มีความหมายหรือเป็นสินค้าชนิดเดียวกัน ให้อยู่ในแถว (Row) เดียวกันอย่างถูกต้องตรงตามรหัสเอกสาร (v1, v2...) ที่ระบุในป้ายคั่น
...
กฎเหล็ก:
- ชื่อคีย์ต้องตรงตามโครงสร้างรหัสย่อ v1, v2... เป๊ะๆ และห้ามสลับข้อมูลเอกสารกันเด็ดขาด
...
- ตรวจสอบตัวเลขให้ถูกต้องแม่นยำ 100% ตามหน้าเอกสาร"""
                ...
                for idx, file in enumerate(uploaded_files):
                    ...
                    # แนบป้ายคั่นรหัสย่อ v1, v2 ให้ AI อ่านอย่างชัดเจน
                ...
                # 5. ประมวลผลราคาหลังหักส่วนลด และคำนวณ Amount (อ้างอิงจากรหัส v1, v2...)
                ...
                for idx, v_id in enumerate(vendor_ids):
                    ...
                    # เก็บข้อมูลลง Key ตามชื่อบริษัทจริงที่ผู้ใช้กรอกเพื่อแสดงตาราง
                ...
                # 9. ฟังก์ชันไฮไลท์สีเขียวเจ้าที่ถูกที่สุดและจัดสีพื้นหลังไม่ให้ติดดำทึบ
